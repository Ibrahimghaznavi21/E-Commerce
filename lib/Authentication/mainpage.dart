...
class _mainpageState extends State<mainpage> {
  ...
  void initState(){
    ...
    final ref = FirebaseFirestore.instance.collection('Users').doc(Global.currentuserid);
    ref.get().then((DocumentSnapshot snapshot) async{
      data = snapshot.data() as Map<String,dynamic>;
      await API.updateToken(Global.currentuserid);

    }).onError((error, stackTrace) => null);
    Provider.of<Allcategories>(context,listen: false).fetchallcatagories().then((value) {
      updateScreen();
    });
        ...
        children: [
          ...
          ListTile(
            leading: TextButton.icon(
              onPressed: () {
                Navigator.push(context, MaterialPageRoute(builder: (c)=> ProfileScreen(data: data,)));
              },
              icon: Icon(
                Icons.person,
                size: 24.0,
          ...
          ListTile(
            leading: TextButton.icon(
              onPressed: () {
                Navigator.push(context, MaterialPageRoute(builder: (c)=> const MyOrderScreen()));
              },
              icon: Icon(
                Icons.shopping_cart,
                size: 24.0,
  ...
  Widget build(BuildContext context) {
    if(!isSearching){
      ...
      ci = Provider.of<Allcategories>(context,listen: false).search(searchtext);
    }
    ...
    Scaffold(
      ...
      appBar: AppBar(
        ...
        title: !isSearching? Row(
          ...
          children: [
          Icon(Icons.card_giftcard),Text(" Mod Graphics",
        style: TextStyle(
        color: Colors.white,
        fontSize: 25,
        fontFamily: "Lobster",
        ...
        actions: [
          ...
          Stack(
            children: [
              ...
              Badge(
                child: Text(
                  Provider.of<cartItems>(context, listen: true).cartitemscount().toString(),
                  style: TextStyle(
                    color: Colors.white
                  ),
                ),
          ...
          child: ListView(
            ...
            children: [
              ...
              Padding(
                ...
                child: const Text("Advertisements",style: TextStyle(fontSize: 25,fontFamily: "Lobster",),),
              ),
                CarouselSlider(
                items: [

                  ...Provider.of<Allcategories>(context,listen: false).adv.map((e) {
            ...
            //Slider Container properties
              ...
              Padding(
                padding: const EdgeInsets.all(8.0),
                child: const Text("Categories",style: TextStyle(fontSize: 25),),
              ),
              Padding(padding: EdgeInsets.symmetric(
                ...
                child: GridView.builder(
                  ...
                  itemBuilder: (context,index){
                    return singlecategory_widget(categoryid: ci[index].categoryid , categoryname: ci[index].categoryname,categoryimage: ci[index].categoryimage,);
                  },
                ),
              )
            ],
